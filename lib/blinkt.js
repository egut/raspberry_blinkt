...
const debug = require('debug')('app:blinkt');
...
const blinkt = function (colors) {
  ...
  colors.slice(-8).forEach((color, i) => {
    debug('LED %d: %d %d %d %d', 7 - i, color[0], color[1], color[2], color[3]);
    leds.setPixel(7 - i, color[0], color[1], color[2], color[3]);
  });

  for (let i = 7 - colors.length; i >= 0; i--) {
    debug('LED %d: %d %d %d %d', i, 0, 0, 0, 0);
